...
def _unescape_c_string(s):
    """Best-effort C string unescape for simple escape sequences."""
    try:
        return bytes(s, "utf-8").decode("unicode_escape")
    except Exception:
        return s

def _obf_key_from_string(plaintext, seed):
    """Generate a 4-byte XOR key based on string content and build seed."""
    h = fnv1a_ascii(plaintext, FNV_OFFSET_32, upper=False)
    h = (h ^ (seed & 0xFFFFFFFF)) & 0xFFFFFFFF
    return [ (h >> (8 * i)) & 0xFF for i in range(4) ]

def _xor_encrypt(plaintext, key):
    """Encrypt plaintext (including null terminator) with a 4-byte XOR key."""
    data = plaintext.encode("utf-8") + b"\x00"
    return [ b ^ key[i % 4] for i, b in enumerate(data) ]

def encrypt_strings(source_dir, output_header, seed, dry_run=False):
    """
    Scan source files for OBFSTR_DEFINE(ID, "string") or OBFSTR("string", ID).
    Generate encrypted blobs and write to output_header.
    """
    import re

    strings_to_encrypt = {}
    pattern_define = re.compile(r'OBFSTR_DEFINE\s*\(\s*([A-Za-z_][A-Za-z0-9_]*)\s*,\s*"((?:\\.|[^"\\])*)"\s*\)')
    pattern_obfstr = re.compile(r'OBFSTR\s*\(\s*"((?:\\.|[^"\\])*)"\s*,\s*([A-Za-z_][A-Za-z0-9_]*)\s*\)')
    ...
    for root, _, files in os.walk(source_dir):
        for f in files:
            ...
            for match in pattern_define.findall(content):
                str_id, raw = match[0], match[1]
                plaintext = _unescape_c_string(raw)
                if str_id in strings_to_encrypt and strings_to_encrypt[str_id] != plaintext:
                    raise ValueError(f"OBFSTR_DEFINE ID '{str_id}' has conflicting values")
                strings_to_encrypt[str_id] = plaintext

            for match in pattern_obfstr.findall(content):
                raw, str_id = match[0], match[1]
                plaintext = _unescape_c_string(raw)
                if str_id in strings_to_encrypt and strings_to_encrypt[str_id] != plaintext:
                    raise ValueError(f"OBFSTR ID '{str_id}' has conflicting values")
                strings_to_encrypt[str_id] = plaintext

    lines = []
    lines.append("// Auto-generated by polymorphic_build.py - DO NOT EDIT")
    lines.append("#pragma once")
    lines.append("")
    lines.append("#include <Demon.h>")
    lines.append("")

    for str_id in sorted(strings_to_encrypt.keys()):
        plaintext = strings_to_encrypt[str_id]
        key = _obf_key_from_string(plaintext, seed)
        encrypted = _xor_encrypt(plaintext, key)
        enc_bytes = ", ".join(f"0x{b:02X}" for b in encrypted)
        key_bytes = ", ".join(f"0x{b:02X}" for b in key)
        lines.append(f"static const BYTE OBFSTR_{str_id}_ENC[] = {{ {enc_bytes} }};")
        lines.append(f"static const BYTE OBFSTR_{str_id}_KEY[4] = {{ {key_bytes} }};")
        lines.append(f"#define OBFSTR_{str_id}_LEN {len(encrypted)}")
        lines.append("")
...
def main():
    ...
    if args.dry_run:
        ...
        obf_header = encrypt_strings(
            os.path.join(repo_root, "payloads", "Demon"),
            os.path.join(repo_root, "payloads", "Demon", "include", "generated", "ObfStrings.h"),
            seed,
            dry_run=True
        )
        print("\n=== ObfStrings.h (preview) ===")
        print(obf_header[:2000])
    else:
        ...
        # Generate ObfStrings.h
        obf_path = os.path.join(out_dir, "payloads", "Demon", "include", "generated", "ObfStrings.h") if args.output_dir else os.path.join(repo_root, "payloads", "Demon", "include", "generated", "ObfStrings.h")
        encrypt_strings(os.path.join(repo_root, "payloads", "Demon"), obf_path, seed, dry_run=False)
